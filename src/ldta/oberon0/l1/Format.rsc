module ldta::oberon0::l1::Format

import ldta::oberon0::l1::AST;
import ldta::oberon0::l1::Priorities;
...
public list[Box] types2boxes(list[TypeDecl] types) =
   	(types != []) ? [KW(L("TYPE")), I([A([typeDecl2box(x) | x <- types])[@vs=0]])[@vs=0]]
   	 : [];

public list[Box] vars2boxes(list[VarDecl] vars) =
    (vars != []) ? [KW(L("VAR")), I([A([varDecl2box(x) | x <- vars])[@vs=0]])[@vs=0]]
   	 : [];


public Box constDecl2box(ConstDecl cd) =
  R([L(cd.name.name), L("="), H([exp2box(cd.\value), L(";")])[@hs=0]]);

public Box typeDecl2box(TypeDecl td) =
  R([L(td.name.name), L(":"), H([type2box(td.\type), L(";")])[@hs=0] ]);

public Box varDecl2box(VarDecl vd) =
  R([H(hsepList(vd.names, ",", id2box))[@hs=1], L(":"), H([type2box(vd.\type), L(";")])[@hs=0]]);
